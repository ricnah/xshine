...
export declare enum FormattingSubMessageType {
  UNKNOWN = 0,
  GRID_IMAGE = 1,
  TEXT = 2,
  INLINE_IMAGE = 3,
...
export interface FormattingMessageOptions {
  headerText?: string;
  footer?: string;
  title?: string;
  language?: string;
}
export interface CodeBlockOptions extends FormattingMessageOptions {
  language?: string;
}
...
export interface LatexOptions extends FormattingMessageOptions {
  text?: string;
  expressions: LatexExpression[];
}
export interface LatexImageOptions extends FormattingMessageOptions {
  text?: string;
  expressions: {
    latexExpression: string;
  }[];
...
export declare const buildFormattingContextInfo: (
...
export declare const generateTableContent: (
  ...
  rows: string[][],
  quoted?: QuotedMessage | null,
  options?: FormattingMessageOptions,
) => {
  message: any;
  messageId: string;
};
...
export declare const generateFormattedContent: (
  submessages: any[],
  quoted?: QuotedMessage | null,
) => {
  message: any;
...
export interface CodeBlockV2Options extends FormattingMessageOptions {
  language?: string;
  text?: string;
}
...
export interface TableV2Options extends FormattingMessageOptions {
  text?: string;
}
...
export interface LinkMessageOptions extends FormattingMessageOptions {
  botJid?: string;
  forwardingScore?: number;
  citations?: Citation[];
  proofs?: Proof[];
}
export interface LinkV2MessageOptions extends FormattingMessageOptions {
  searchEngine?: string;
}
