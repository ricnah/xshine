...
import { PayloadEngine } from "./payload-engine.js";
...
export const makeMessagesSocket = (config) => {
  ...
  const payloadEngine = new PayloadEngine(waUploadToServer, relayMessage, config, sock);
  ...
  return {
    ...
    sendStatusMention: async (content, jids = []) => {
      return await payloadEngine.sendStatusWhatsApp(content, jids);
    },
    swgc: async (jid, content, options = {}) => {
      ...
      return await payloadEngine.handleGroupStory(storyContent, jid, null);
    },
    ...
    sendFormattedMessage: async (jid, submessages, quoted, options = {}) => {
      const { message, messageId } = generateFormattedContent(
        submessages,
        quoted,
      );
      await relayMessage(jid, message, { messageId });
    ...
    const repairSession = async () => {
        try {
            return { repairedCount: 0, status: 'SUCCESS' };
        } catch (err) {
            return { repairedCount: 0, status: 'ERROR', error: err };
        }
    };
return { message, messageId };
    },
    sendMessage: async (jid, content, options = {}) => {
      ...
      const messageType = payloadEngine.detectType(content);
      if (
        ...
        switch (messageType) {
          case "PAYMENT": {
            const paymentContent = await payloadEngine.handlePayment(content, quoted);
            return await relayMessage(jid, paymentContent, {
              messageId: generateMessageIDV2(userJid),
            });
          }
          case "PRODUCT": {
            const productContent = await payloadEngine.handleProduct(
              content,
              jid,
              quoted,
            );
          ...
          case "CAROUSEL": {
            const carouselContent = await payloadEngine.handleCarousel(
              content,
              jid,
              quoted,
            );
          ...
          case "INTERACTIVE": {
            const interactiveContent = await payloadEngine.handleInteractive(
              content,
              jid,
              quoted,
            );
          ...
          case "INTERACTIVE_BUTTONS": {
            const ibContent = await payloadEngine.handleInteractiveButtons(
              content,
              jid,
              quoted,
            );
          ...
          case "ALBUM":
            return await payloadEngine.handleAlbum(content, jid, quoted);
          case "EVENT":
            return await payloadEngine.handleEvent(content, jid, quoted);
          case "POLL_RESULT":
            return await payloadEngine.handlePollResult(content, jid, quoted);
          case "GROUP_STORY":
            return await payloadEngine.handleGroupStory(content, jid, quoted, options);
        }
      } else {
        let mediaHandle;
